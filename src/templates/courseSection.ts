type CourseSectionsProps = {
  ...
  title: string
  description: string
  lessonsList: {name:string}[]
}

export const CourseSection = (config:CourseSectionsProps) => `
<div class="overflow-hidden bg-white py-10 ">
  <div class="mx-auto max-w-7xl px-6 lg:px-8">
    <div class="mx-auto max-w-2xl gap-11 lg:gap-4 lg:mx-0 lg:max-w-none flex flex-col ${config?.invert ? 'lg:flex-row-reverse' : 'lg:flex-row'} ">
      
      <div class="lg:pt-4 w-full">
        <div class="lg:max-w-lg">
          <h2 class="text-base font-semibold leading-7 text-indigo-600">${config?.levelLabel}</h2>
          <p class="mt-2 text-3xl font-bold tracking-tight text-gray-900 sm:text-4xl">${config?.title}</p>
          <p class="mt-6 text-lg leading-8 text-gray-600">${config?.description}</p>
          <dl class="mt-10 max-w-xl space-y-6 text-base leading-7 text-gray-600 lg:max-w-none">
            
            ${(()=>{
              let lessonsString = ``
              for(let [index,lesson] of config?.lessonsList.entries()) {
                lessonsString += `
                  <div class="relative pl-9  ">
                    <div class="border-b-2 border-[#ddd] border-dashed">
                      <dt class="inline-flex items-start font-semibold text-gray-900 ">
                        <span class="text-indigo-600 text-xl mr-1 w-[2em] inline-block">${index+1<10 && '0'}${index+1}</span>
                        ${lesson.name}
                      </dt>
                    </div>
                  </div>
      ...
      <div class="w-full h-[300px] sm:h-[400px] lg:h-auto bg-orange-500">
        image
      </div>
